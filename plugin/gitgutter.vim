...
function! s:is_tracked_by_git()
  if (s:vcs == 'git')
    ...
    return !v:shell_error
  elseif (s:vcs == 'hg')
    let cmd = 'hg status -i ' . shellescape(s:file()) . ' | grep -e "^\I "' . s:discard_stdout_and_stderr()
    call system(s:command_in_directory_of_file(cmd))
    return v:shell_error
  endif
endfunction
...
function! s:run_diff()
  if (s:vcs == 'git')
    let cmd = 'git diff --no-ext-diff --no-color -U0 ' . g:gitgutter_diff_args . ' ' . shellescape(s:current_file()) .
                \ ' | grep -e "^@@ "'
  elseif (s:vcs == 'hg')
